...
router.get('/pdf', async (req, res) => {
    try {
        ...
        const sectionRow = await db_1.pool.query(`SELECT s.id as section_id, s.label as section_label, u.name as teacher_name
       FROM sections s
       ...
       LIMIT 1`, [user_id, school_id]);
        ...
        const settlingText = req.query.settling_text;
        let pdfBuffer;
        try {
            const aiResp = await axios_1.default.post(`${AI()}/internal/export-pdf`, { teacher_id: user_id, section_id, section_label, teacher_name, date, days: 1, settling_text: settlingText || null }, { responseType: 'arraybuffer', timeout: 15000 });
