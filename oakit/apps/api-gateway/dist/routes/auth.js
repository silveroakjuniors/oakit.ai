...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!school_code || school_code === 'platform') {
            const saResult = await db_1.pool.query(`SELECT id, password_hash, is_active, role FROM users
         WHERE (email = $1 OR mobile = $1) AND school_id IS NULL AND role = 'super_admin'`, [identifier]);
            if (saResult.rows.length > 0) {
                const sa = saResult.rows[0];
                if (!sa.is_active || !sa.password_hash) {
                    return res.status(401).json({ error: 'Invalid credentials' });
                }
                const valid = await bcryptjs_1.default.compare(password, sa.password_hash);
                if (!valid)
                    return res.status(401).json({ error: 'Invalid credentials' });
                const token = (0, jwt_1.signToken)({ user_id: sa.id, school_id: null, role: 'super_admin', permissions: [] });
        ...
        // Find school
        const schoolResult = await db_1.pool.query('SELECT id, status FROM schools WHERE subdomain = $1', [school_code.toLowerCase()]);
...
router.post('/change-password', auth_1.jwtVerify, async (req, res) => {
    try {
        ...
        await db_1.pool.query('UPDATE users SET password_hash = $1, force_password_reset = false WHERE id = $2', [hash, user_id]);
...
router.get('/security-questions', async (_req, res) => {
    try {
        const result = await db_1.pool.query('SELECT id, text FROM security_questions ORDER BY text');
...
router.post('/setup-security-question', auth_1.jwtVerify, async (req, res) => {
    try {
        ...
        const hash = await bcryptjs_1.default.hash(answer.trim().toLowerCase(), 12);
        await db_1.pool.query('UPDATE users SET security_question_id = $1, security_answer_hash = $2 WHERE id = $3', [security_question_id, hash, user_id]);
...
router.post('/forgot-password/init', async (req, res) => {
    try {
        ...
        const schoolResult = await db_1.pool.query('SELECT id FROM schools WHERE subdomain = $1', [school_code.toLowerCase()]);
        if (schoolResult.rows.length === 0) {
            return res.status(401).json({ error: 'Invalid credentials' });
        }
        const school_id = schoolResult.rows[0].id;
        const userResult = await db_1.pool.query(`SELECT u.id, sq.text as question_text
       FROM users u
       LEFT JOIN security_questions sq ON u.security_question_id = sq.id
       WHERE u.mobile = $1 AND u.school_id = $2`, [mobile, school_id]);
        if (userResult.rows.length === 0) {
            return res.status(401).json({ error: 'Invalid credentials' });
        }
        const user = userResult.rows[0];
...
router.post('/forgot-password/verify', async (req, res) => {
    try {
        ...
        const userResult = await db_1.pool.query('SELECT security_answer_hash FROM users WHERE id = $1', [user_id]);
        if (userResult.rows.length === 0) {
        ...
        if (!security_answer_hash)
            return res.status(401).json({ error: 'Incorrect answer' });
        const valid = await bcryptjs_1.default.compare(answer.trim().toLowerCase(), security_answer_hash);
        if (!valid)
            return res.status(401).json({ error: 'Incorrect answer' });
        // Issue short-lived reset token
        const import_jwt = require('jsonwebtoken');
        const resetToken = import_jwt.sign({ user_id, purpose: 'password_reset' }, process.env.JWT_SECRET || 'change_me', { expiresIn: '15m' });
...
router.post('/forgot-password/reset', async (req, res) => {
    try {
        ...
        const userRow = await db_1.pool.query('SELECT mobile FROM users WHERE id = $1', [payload.user_id]);
        if (userRow.rows.length === 0)
            return res.status(404).json({ error: 'User not found' });
        ...
        const hash = await bcryptjs_1.default.hash(new_password, 12);
        await db_1.pool.query('UPDATE users SET password_hash = $1, force_password_reset = false WHERE id = $2', [hash, payload.user_id]);
...
router.post('/setup', async (req, res) => {
    try {
        ...
        const userResult = await db_1.pool.query('SELECT id FROM users WHERE setup_token = $1 AND setup_expires > now()', [setup_token]);
        if (userResult.rows.length === 0) {
            return res.status(400).json({ error: 'Invalid or expired setup token' });
        }
        const hash = await bcryptjs_1.default.hash(password, 12);
        await db_1.pool.query('UPDATE users SET password_hash = $1, setup_token = NULL, setup_expires = NULL WHERE id = $2', [hash, userResult.rows[0].id]);
