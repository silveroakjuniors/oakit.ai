 ...
 */
async function _copyPlansToSection(school_id, class_id, new_section_id, teacher_id) {
    // Find a sibling section that already has plans
    const sibling = await db_1.pool.query(`SELECT dp.plan_date, dp.chunk_ids, dp.status
     FROM day_plans dp
     JOIN sections s ON s.id = dp.section_id
     WHERE s.class_id = $1 AND s.school_id = $2 AND dp.section_id != $3
     ORDER BY dp.plan_date`, [class_id, school_id, new_section_id]);
    if (sibling.rows.length === 0)
        return 0;
    let copied = 0;
    for (const plan of sibling.rows) {
        await db_1.pool.query(`INSERT INTO day_plans (school_id, section_id, teacher_id, plan_date, chunk_ids, status)
       VALUES ($1, $2, $3, $4, $5::uuid[], $6)
       ON CONFLICT (section_id, plan_date) DO NOTHING`, [school_id, new_section_id, teacher_id, plan.plan_date, plan.chunk_ids, plan.status]);
        copied++;
    }
    return copied;
}
...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT c.id, c.name,
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('INSERT INTO classes (school_id, name) VALUES ($1, $2) RETURNING id, name', [school_id, name]);
...
router.get('/:id/sections', async (req, res) => {
    try {
        const { school_id } = req.user;
        const result = await db_1.pool.query(`SELECT s.id, s.label,
        COALESCE(json_agg(
       ...
       GROUP BY s.id
       ORDER BY s.label`, [req.params.id, school_id]);
        return res.json(result.rows);
    }
    catch (err) {
        return res.status(500).json({ error: 'Internal server error' });
...
router.post('/:id/sections', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query('INSERT INTO sections (school_id, class_id, label) VALUES ($1, $2, $3) RETURNING id, label', [school_id, req.params.id, label]);
        ...
        const anyTeacher = await db_1.pool.query(`SELECT COALESCE(s.class_teacher_id, ts.teacher_id) as teacher_id
       FROM sections s
       LEFT JOIN teacher_sections ts ON ts.section_id = s.id
       WHERE s.class_id = $1 AND s.school_id = $2 AND s.id != $3
         AND COALESCE(s.class_teacher_id, ts.teacher_id) IS NOT NULL
       LIMIT 1`, [req.params.id, school_id, newSection.id]);
        let plansCopied = 0;
...
router.post('/sections/:id/teachers', async (req, res) => {
    try {
        ...
        await db_1.pool.query('INSERT INTO teacher_sections (teacher_id, section_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [teacher_id, req.params.id]);
...
router.delete('/sections/:id/teachers/:tid', async (req, res) => {
    try {
        await db_1.pool.query('DELETE FROM teacher_sections WHERE section_id = $1 AND teacher_id = $2', [req.params.id, req.params.tid]);
...
router.patch('/:id/timings', async (req, res) => {
    try {
        const { school_id } = req.user;
        const { day_start_time, day_end_time } = req.body;
        if (!day_start_time || !day_end_time)
            return res.status(400).json({ error: 'day_start_time and day_end_time are required' });
        await db_1.pool.query('UPDATE classes SET day_start_time = $1, day_end_time = $2 WHERE id = $3 AND school_id = $4', [day_start_time, day_end_time, req.params.id, school_id]);
