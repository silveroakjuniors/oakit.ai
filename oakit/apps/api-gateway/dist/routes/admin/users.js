...
router.get('/roles', async (req, res) => {
    try {
        const { school_id } = req.user;
        const result = await db_1.pool.query('SELECT id, name, permissions FROM roles WHERE school_id = $1 ORDER BY name', [school_id]);
        return res.json(result.rows);
    }
    catch (err) {
        return res.status(500).json({ error: 'Internal server error' });
...
router.post('/roles', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`INSERT INTO roles (school_id, name, permissions) VALUES ($1, $2, $3)
       ON CONFLICT (school_id, name) DO UPDATE SET permissions = EXCLUDED.permissions
       RETURNING id, name, permissions`, [school_id, name.toLowerCase().trim(), JSON.stringify(permissions || [])]);
...
router.put('/roles/:id', async (req, res) => {
    try {
        const { school_id } = req.user;
        const { name, permissions } = req.body;
        const result = await db_1.pool.query(`UPDATE roles SET name = COALESCE($1, name), permissions = COALESCE($2, permissions)
       WHERE id = $3 AND school_id = $4 RETURNING id, name, permissions`, [name || null, permissions ? JSON.stringify(permissions) : null, req.params.id, school_id]);
