...
router.get('/pools', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `SELECT ap.id, ap.name, ap.description, ap.language, ap.created_at,
...
router.post('/pools', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO activity_pools (school_id, name, description, language)
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [school_id, name, description ?? null, language]
    );
    return res.status(201).json(result.rows[0]);
  } catch (err: any) {
    if (err.code === '23505') return res.status(400).json({ error: 'A pool with this name already exists' });
...
router.delete('/pools/:id', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    await pool.query(
      'DELETE FROM activity_pools WHERE id = $1 AND school_id = $2',
...
router.get('/pools/:pool_id/activities', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    const result = await pool.query(
...
router.post('/pools/:pool_id/activities', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    ...
    const poolRow = await pool.query(
      'SELECT id FROM activity_pools WHERE id = $1 AND school_id = $2',
      [req.params.pool_id, school_id]
    );
    if (poolRow.rows.length === 0) return res.status(404).json({ error: 'Pool not found' });

    // Get next position
    const posRow = await pool.query(
      'SELECT COALESCE(MAX(position), -1) + 1 AS next_pos FROM activities WHERE activity_pool_id = $1',
      [req.params.pool_id]
    );
    const position = posRow.rows[0].next_pos;

    const result = await pool.query(
      `INSERT INTO activities (activity_pool_id, school_id, title, description, position)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [req.params.pool_id, school_id, title, description ?? null, position]
    );
    return res.status(201).json(result.rows[0]);
  } catch (err: any) {
    if (err.code === '23505') return res.status(400).json({ error: 'An activity with this title already exists in this pool' });
    return res.status(500).json({ error: 'Internal server error' });
  }
});

// DELETE /pools/:pool_id/activities/:id
router.delete('/pools/:pool_id/activities/:id', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    // Check if activity has plans
    const inUse = await pool.query(
      'SELECT id FROM supplementary_plans WHERE activity_id = $1 LIMIT 1',
      [req.params.id]
    );
    ...
    await pool.query(
      `DELETE FROM activities WHERE id = $1 AND school_id = $2`,
      [req.params.id, school_id]
    );
...
router.put('/pools/:pool_id/activities/reorder', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    ...
    try {
      ...
      for (const item of order) {
        await client.query(
          'UPDATE activities SET position = $1 WHERE id = $2 AND school_id = $3',
          [item.position, item.id, school_id]
        );
      }
...
router.get('/assignments', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    const result = await pool.query(
      `SELECT pa.id, pa.activity_pool_id, pa.class_id, pa.frequency_mode,
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO pool_assignments
       ...
       RETURNING *`,
      [school_id, activity_pool_id, class_id, frequency_mode, interval_days ?? null, start_date, end_date, carry_forward_on_miss ?? false]
    );
    ...
    if (err.code === '23505') return res.status(409).json({ error: 'This pool is already assigned to this class' });
...
router.delete('/assignments/:id', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    await pool.query(
      'UPDATE pool_assignments SET is_deleted = true WHERE id = $1 AND school_id = $2',
      [req.params.id, school_id]
    );
...
router.post('/assignments/:id/reschedule', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    const assignmentRow = await pool.query(
      'SELECT * FROM pool_assignments WHERE id = $1 AND school_id = $2 AND is_deleted = false',
      [req.params.id, school_id]
    );
    if (assignmentRow.rows.length === 0) return res.status(404).json({ error: 'Assignment not found' });
...
router.get('/schedule', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `SELECT sp.id, sp.section_id, sp.plan_date, sp.status, sp.override_note,
       ...
       ORDER BY sp.plan_date, s.name, ap.name`,
      [school_id, class_id, startDate, endDate]
    );
    return res.json(result.rows);
  } catch (err) {
    return res.status(500).json({ error: 'Internal server error' });
...
async function scheduleAssignment(assignment: any, school_id: string) {
  ...
  const sections = sectionsResult.rows.map((r: any) => r.id);
  if (sections.length === 0) return;

  // Get all activities in order
  const activitiesResult = await pool.query(
    'SELECT id FROM activities WHERE activity_pool_id = $1 ORDER BY position, created_at',
    [activity_pool_id]
  );
  const activityIds: string[] = activitiesResult.rows.map((r: any) => r.id);
  if (activityIds.length === 0) return;

  // Get school calendar (working days + holidays)
  const calendarResult = await pool.query(
    `SELECT working_days, holidays, start_date AS cal_start, end_date AS cal_end
  ...
  const workingDays: number[] = calendar?.working_days || [1, 2, 3, 4, 5];
  const holidays: string[] = (calendar?.holidays || []).map((d: any) => String(d).split('T')[0]);
  ...
  const specialDays = new Set(specialResult.rows.map((r: any) => String(r.day_date).split('T')[0]));
  ...
  for (const section_id of sections) {
    ...
    const cursorResult = await pool.query(
      `INSERT INTO supplementary_rotation_cursors (section_id, pool_assignment_id, next_position)
       VALUES ($1, $2, 0)
    ...
    for (const planDate of scheduledDates) {
      ...
      await pool.query(
        `INSERT INTO supplementary_plans
           (school_id, section_id, pool_assignment_id, activity_id, plan_date)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (section_id, pool_assignment_id, plan_date) DO NOTHING`,
        [school_id, section_id, assignment_id, activityId, planDate]
      );
    }

    // Update cursor
    await pool.query(
      `UPDATE supplementary_rotation_cursors SET next_position = $1
       WHERE section_id = $2 AND pool_assignment_id = $3`,
      [cursor, section_id, assignment_id]
    );
  }
}
