...
def _build_chunk_context(chunks) -> str:
    ...
    parts = []
    for chunk in chunks:
        block = f"Topic: {chunk['topic_label'] or 'Untitled'}\n"
        block += f"Content:\n{chunk['content'].strip()}"
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if result:
                        print(f"[LLM] Gemini responded successfully")
...
def _format_rich_plan(chunks, date_label: str, class_full: str, carried_note: str, pending_rows, age_info: dict) -> str:
    lines = [f"📅 **Your plan for {date_label}**\n{class_full} | {age_info['age']}\n"]
    if carried_note:
        lines.append(f"⚠️ *{carried_note}*\n")
    lines.append("---\n")
    ...
    for i, chunk in enumerate(chunks):
        ...
        for subj in subjects:
            guide = _get_conduct_guide(subj["subject"])
            icon  = guide.get("icon", "•")
            lines.append(f"**{icon} {subj['subject']}**")
            lines.append(f"*Curriculum:* {subj['activity']}\n")

            if guide:
                lines.append(f"🎯 *Objective:* {guide['objective']}\n")
                lines.append("📋 **How to conduct:**")
                ...
                tip = _get_tip(subj["subject"])
                if tip:
                    lines.append(f"💡 {tip}\n")
            lines.append("---")

        if chunk["activity_ids"]:
            lines.append(f"📎 **Materials:** {', '.join(chunk['activity_ids'])}\n")

    if pending_rows:
        labels = ", ".join(r["topic_label"] or "Topic" for r in pending_rows[:3])
        lines.append(f"\n⏳ **Pending from previous days:** {labels}")
        lines.append("Try to fit these in today if time allows.\n")

    lines.append("\n---")
    lines.append("💬 Ask me: *\"How do I conduct Math today?\"* or *\"What questions should I ask during English?\"*")
    return "\n".join(lines)
...
async def query(teacher_id: str, school_id: str, text: str, query_date: str, role: str = "teacher", history: list = None) -> dict:
    try:
        pool = await get_pool()
...
- Total response under 300 words"""
            ...
            response_text, llm_provider = await _call_llm(llm_prompt, system_prompt)
...
- Under 200 words"""
                ...
                response_text, llm_provider = await _call_llm(llm_prompt, system_prompt)
                if not response_text:
                    # LLM unavailable — use context-aware fallback
                    ...
                    elif day_ctx["day_type"] == "curriculum" and chunks:
                        response_text = _curriculum_aware_fallback(text, list(chunks), class_name, age_info, date_label)
                    else:
                        # Special day fallback — build a structured response from guidance text
                        response_text = (
                            f"**{day_ctx['label']} — {class_full} ({age_info['age']})**\n\n"
                            + day_ctx["curriculum_context"]
                            + (f"\n\n📝 **Admin note:** {day_ctx['activity_note']}" if day_ctx["activity_note"] else "")
                            + f"\n\n{_get_classroom_mgmt_advice(text, class_name, age_info)}"
                        )
            else:
