...
interface Invoice {
  ...
  period_from: string;
  period_to: string;
  line_items: { description: string; quantity: number; unit_price_paise: number; amount_paise: number }[];
...
export default function PlatformBillingPage() {
  ...
  async function handleGenerateInvoice(e: React.FormEvent) {
    ...
    setGenerating(true);
    try {
      const inv = await apiPost<Invoice>(`/api/v1/super-admin/platform-billing/${selected.id}/invoices`, invoiceForm, token);
      setInvoices(prev => [inv, ...prev]);
      flash(`✓ Invoice ${inv.invoice_number} generated (₹${inr(inv.total_paise)})`);
      setTab('invoices');
    } catch (e: any) { flash(e.message, true); }
  ...
  // Platform totals
  const totalMonthlyRevenue = schools.reduce((s, sc) => s + parseFloat(sc.monthly_revenue_inr), 0);
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { label: 'Monthly Revenue', value: `₹${totalMonthlyRevenue.toFixed(2)}`, color: 'text-emerald-400' },
        ...
        <div className="rounded-2xl overflow-hidden mb-6" style={DARK}>
          <table className="w-full text-sm">
            <thead style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              <tr>
                {['School', 'Students', 'Per Student', 'Monthly', 'Outstanding', 'GST', ''].map(h => (
            ...
            <tbody>
              {schools.map(s => (
                <tr key={s.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <td className="px-4 py-3 text-emerald-400 font-semibold">₹{s.monthly_revenue_inr}</td>
                  <td className="px-4 py-3">
                        ...
                        <div key={inv.id} className="rounded-2xl p-4" style={DARK}>
                          <div className="flex items-start justify-between gap-3 mb-3">
                            ...
                            <div className="text-right">
                              <p className="font-bold text-white">₹{inr(inv.total_paise)}</p>
                          ...
                          <div className="space-y-1 mb-3">
                            {inv.line_items.map((li, i) => (
                              <div key={i} className="flex justify-between text-xs text-white/50">
                                ...
                                <span>₹{inr(li.amount_paise)}</span>
                              </div>
                            ))}
                            {inv.gst_enabled && inv.gst_paise > 0 && (
                              <div className="flex justify-between text-xs text-white/50">
                                <span>GST ({inv.gst_percentage}%)</span>
                                <span>₹{inr(inv.gst_paise)}</span>
                              </div>
                            )}
                            <div className="flex justify-between text-xs font-bold text-white border-t border-white/10 pt-1 mt-1">
                              <span>Total</span>
                              <span>₹{inr(inv.total_paise)}</span>
                            </div>
                          </div>
                          {/* Status actions */}
