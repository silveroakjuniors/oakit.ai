...
export default function PrincipalAnnouncementsPage() {
  ...
  function AnnouncementCard({ a }: { a: Announcement }) {
    ...
    const classLabel = a.target_audience === 'class'
      ...
      : null;
    return (
      <Card padding="sm" className={`border-l-4 ${exp ? 'border-neutral-200 opacity-60' : 'border-[#1B4332]'}`}>
        <div
        ...
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                {classLabel ?? (audienceLabel[a.target_audience] ?? a.target_audience)}
              </span>
              {exp && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-500">Expired</span>}
            </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button onClick={e => { e.stopPropagation(); deleteAnn(a.id); }}
              className="text-xs text-red-400 hover:text-red-600 px-2 py-1 rounded-lg hover:bg-red-50 transition-colors ml-1">
              Delete
            </button>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-0.5">
            Broadcast messages to teachers and parents &middot; {active.length} active
          </p>
        </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <select value={form.target_audience}
                onChange={e => setForm(f => ({ ...f, target_audience: e.target.value, target_class_id: '' }))}
