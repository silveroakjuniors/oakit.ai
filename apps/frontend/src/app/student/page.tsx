...
import {
  Home, BookOpen, Sparkles, ClipboardList, TrendingUp,
  LogOut, ChevronLeft, ChevronRight, Send, Loader2,
  CheckCircle2, AlertCircle, Clock, BarChart2, RefreshCw,
} from 'lucide-react';
...
interface HomeworkRecord {
  ...
  status: 'completed' | 'partial' | 'not_submitted';
  teacher_note: string | null;
}
interface ChatMsg { role: 'user' | 'ai'; text: string; ts: number; }
interface QuizTopic { chunk_id: string; topic_name: string; subject: string; covered_date: string; }
interface QuizQuestion {
  id: string;
  question: string;
  q_type: 'fill_blank' | 'descriptive' | '1_mark' | '2_mark';
  marks: number;
}
...
interface ProgressStats {
  ...
  subject_breakdown: { subject: string; avg_pct: number; quiz_count: number }[];
  weak_areas: { subject: string; chapter: string; avg_pct: number }[];
...
type Tab = 'today' | 'homework' | 'ask' | 'quiz' | 'progress';
type QuizStep = 'idle' | 'subject' | 'daterange' | 'topics' | 'qtypes' | 'taking' | 'results';
...
function defaultChat(): ChatMsg[] {
  return [{ role: 'ai', text: "Hi! I'm Oakie 🌳 Ask me anything about topics your class has covered. I'm here to help you understand better!", ts: 0 }];
}
...
export default function StudentPage() {
  ...
  // Ask Oakie tab
  const [chatMsgs, setChatMsgs] = useState<ChatMsg[]>(defaultChat());
  ...
  const [quizTo, setQuizTo] = useState(today());
  const [quizTopics, setQuizTopics] = useState<QuizTopic[]>([]);
  const [selectedTopics, setSelectedTopics] = useState<string[]>([]);
  const [selectedQTypes, setSelectedQTypes] = useState<string[]>(['fill_blank', '1_mark']);
  ...
  // Timer for assigned tests
  useEffect(() => {
    if (activeTestId && testTimer > 0) {
