...
router.get('/teachers', async (req, res) => {
    try {
        ...
        const result = await db_1.pool.query(`SELECT DISTINCT
       ...
       ORDER BY s.name, u.name`, [user_id, school_id]);
...
router.get('/children', async (req, res) => {
    try {
        const { user_id, school_id } = req.user;
        const result = await db_1.pool.query(`SELECT s.id, s.name, s.father_name, s.mother_name,
              s.photo_path,
              c.name as class_name, sec.label as section_label,
       ...
       ORDER BY c.name, s.name`, [user_id, school_id]);
...
router.post('/child/:student_id/photo', (req, res, next) => {
    ...
    try {
        ...
        const link = await db_1.pool.query(`SELECT psl.student_id FROM parent_student_links psl
       JOIN students s ON s.id = psl.student_id
       WHERE psl.parent_id = $1 AND psl.student_id = $2 AND s.school_id = $3`, [user_id, student_id, school_id]);
        ...
        const oldRow = await db_1.pool.query('SELECT photo_path FROM students WHERE id = $1', [student_id]);
        await (0, storage_1.deleteFile)(oldRow.rows[0]?.photo_path);
        // Upload to Supabase
        ...
        await db_1.pool.query('UPDATE students SET photo_path = $1 WHERE id = $2', [storagePath, student_id]);
...
router.get('/child/:student_id/feed', async (req, res) => {
    try {
        const { user_id, school_id } = req.user;
        const { student_id } = req.params;
        // Verify parent is linked to this student
       ...
       WHERE psl.parent_id = $1 AND psl.student_id = $2 AND s.school_id = $3`, [user_id, student_id, school_id]);
        ...
        const studentRow = await db_1.pool.query(`SELECT s.section_id, s.name, c.name as class_name, sec.label as section_label
       ...
       WHERE s.id = $1`, [student_id]);
        ...
        // Today's attendance
        const attRow = await db_1.pool.query(`SELECT status, is_late, arrived_at FROM attendance_records
       WHERE student_id = $1 AND attend_date = $2 LIMIT 1`, [student_id, today]);
        // Today's completion
        const compRow = await db_1.pool.query(`SELECT dc.covered_chunk_ids, dc.submitted_at,
              u.name as teacher_name
       FROM daily_completions dc
       JOIN users u ON u.id = dc.teacher_id
       WHERE dc.section_id = $1 AND dc.completion_date = $2 LIMIT 1`, [section_id, today]);
        // Today's plan topics
        const planRow = await db_1.pool.query(`SELECT dp.chunk_ids, dp.status, sd.label as special_label, sd.day_type
       FROM day_plans dp
       LEFT JOIN special_days sd ON sd.school_id = dp.school_id AND sd.day_date = dp.plan_date
       WHERE dp.section_id = $1 AND dp.plan_date = $2 LIMIT 1`, [section_id, today]);
        let topics = [];
        if (planRow.rows.length > 0 && planRow.rows[0].chunk_ids?.length > 0) {
            const chunks = await db_1.pool.query('SELECT topic_label FROM curriculum_chunks WHERE id = ANY($1::uuid[]) ORDER BY chunk_index', [planRow.rows[0].chunk_ids]);
            topics = chunks.rows.map((r) => r.topic_label);
        }
        // Today's homework
        const hwRow = await db_1.pool.query(`SELECT formatted_text, raw_text FROM teacher_homework
       WHERE section_id = $1 AND homework_date = $2 LIMIT 1`, [section_id, today]).catch(() => ({ rows: [] }));
        // Today's notes (non-expired, deduplicated by content)
        const notesRow = await db_1.pool.query(`SELECT DISTINCT ON (COALESCE(note_text, ''), COALESCE(file_name, ''))
              id, note_text, file_name, file_size, expires_at, created_at
       FROM teacher_notes
       WHERE section_id = $1 AND note_date = $2 AND expires_at > now()
       ORDER BY COALESCE(note_text, ''), COALESCE(file_name, ''), created_at DESC`, [section_id, today]).catch(() => ({ rows: [] }));
        return res.json({
...
router.get('/child/:student_id/attendance', async (req, res) => {
    try {
        const { user_id, school_id } = req.user;
        const { student_id } = req.params;
        const link = await db_1.pool.query(`SELECT 1 FROM parent_student_links psl JOIN students s ON s.id = psl.student_id
       WHERE psl.parent_id = $1 AND psl.student_id = $2 AND s.school_id = $3`, [user_id, student_id, school_id]);
        if (link.rows.length === 0)
            return res.status(403).json({ error: 'Not authorized' });
        const result = await db_1.pool.query(`SELECT attend_date::text, status, is_late, arrived_at
       FROM attendance_records WHERE student_id = $1
       ORDER BY attend_date DESC LIMIT 60`, [student_id]);
...
router.get('/absences', async (req, res) => {
    try {
        const { user_id, school_id } = req.user;
        // Get linked students
        const links = await db_1.pool.query(`SELECT psl.student_id, s.name as student_name
       FROM parent_student_links psl
       JOIN students s ON s.id = psl.student_id
       WHERE psl.parent_id = $1`, [user_id]);
        const absences = [];
        for (const link of links.rows) {
            const absenceRows = await db_1.pool.query(`SELECT ar.attend_date, ar.status
         FROM attendance_records ar
         WHERE ar.student_id = $1 AND ar.status = 'absent'
         ORDER BY ar.attend_date DESC`, [link.student_id]);
            for (const absence of absenceRows.rows) {
                // Get covered chunks for that date in the student's section
                const sectionRow = await db_1.pool.query('SELECT section_id FROM students WHERE id = $1', [link.student_id]);
                const section_id = sectionRow.rows[0]?.section_id;
                let covered_chunks = [];
                if (section_id) {
                    const completion = await db_1.pool.query('SELECT covered_chunk_ids FROM daily_completions WHERE section_id = $1 AND completion_date = $2', [section_id, absence.attend_date]);
                    if (completion.rows.length > 0 && completion.rows[0].covered_chunk_ids?.length > 0) {
                        const chunks = await db_1.pool.query('SELECT id, topic_label FROM curriculum_chunks WHERE id = ANY($1::uuid[])', [completion.rows[0].covered_chunk_ids]);
...
router.get('/notes/:id/download', async (req, res) => {
    try {
        ...
        const noteRow = await db_1.pool.query(`SELECT tn.file_path, tn.file_name, tn.expires_at, tn.section_id
       FROM teacher_notes tn
       WHERE tn.id = $1 AND tn.school_id = $2`, [req.params.id, school_id]);
        if (noteRow.rows.length === 0)
            return res.status(404).json({ error: 'Note not found' });
        const note = noteRow.rows[0];
        // Check parent has a child in this section
        const authRow = await db_1.pool.query(`SELECT 1 FROM parent_student_links psl
       JOIN students s ON s.id = psl.student_id
       WHERE psl.parent_id = $1 AND s.section_id = $2 LIMIT 1`, [user_id, note.section_id]);
        ...
        const publicUrl = (0, storage_1.getPublicUrl)(note.file_path);
        if (!publicUrl)
...
router.post('/missed-topics/:id/done', async (req, res) => {
    try {
        const { user_id } = req.user;
        const result = await db_1.pool.query(`UPDATE missed_topic_tasks SET is_done = true, done_at = now()
       WHERE id = $1 AND parent_id = $2
       RETURNING *`, [req.params.id, user_id]);
...
router.get('/missed-topics/completed', async (req, res) => {
    try {
        const { user_id } = req.user;
        const result = await db_1.pool.query(`SELECT mtt.*, cc.topic_label, s.name as student_name
       ...
       ORDER BY mtt.done_at DESC`, [user_id]);
        return res.json(result.rows);
    }
    catch (err) {
        return res.status(500).json({ error: 'Internal server error' });
