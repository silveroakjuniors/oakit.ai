...
router.get('/', async (req, res) => {
    try {
        ...
        const calRow = await db_1.pool.query(`SELECT start_date, end_date, academic_year
       ...
       WHERE psl.parent_id = $1`, [user_id]);
        ...
        for (const link of links.rows) {
            ...
            const totalRow = await db_1.pool.query(`SELECT COUNT(*)::int AS total
         ...
         WHERE sec.id = $1 AND cd.school_id = $2`, [section_id, school_id]);
            ...
            const coveredRow = await db_1.pool.query(`SELECT COUNT(DISTINCT chunk_id)::int AS covered
         FROM (
         ...
         ) sub`, [section_id, yearStart, yearEnd]);
