...
router.get('/', async (req, res) => {
    try {
        ...
        if (section_id) {
            ...
            const mockActive = !!(await redis_1.redis.get(`time_machine:${school_id}`));
            const hour = mockActive ? 9 : new Date().getHours();
            attendance_prompt = attRow.rows.length === 0 && hour >= 7 && hour < 17;
            // Check if today is already completed
            const completionRow = await db_1.pool.query('SELECT id FROM daily_completions WHERE section_id = $1 AND completion_date = $2', [section_id, today]);
            ...
            if (today_completed) {
                ...
                const tmPlan = await db_1.pool.query(`SELECT dp.plan_date, dp.status, dp.chunk_ids,
           ...
           GROUP BY dp.id`, [section_id, tomorrowStr, school_id]);
                if (tmPlan.rows.length > 0) {
                    const row = tmPlan.rows[0];
                    // Check special day for tomorrow
                    if (!row.chunk_ids?.length) {
                        const special = await db_1.pool.query('SELECT label, day_type FROM special_days WHERE school_id=$1 AND day_date=$2 LIMIT 1', [school_id, tomorrowStr]);
        ...
        return res.json({ greeting, thought_for_day, attendance_prompt, today, time_machine_active: !!(await redis_1.redis.get(`time_machine:${school_id}`)), today_completed, tomorrow_plan });
    }
    catch (err) {
