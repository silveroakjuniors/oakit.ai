...
router.get('/today', async (req, res) => {
    try {
       ...
       ORDER BY s.name`, [today, section_id]);
        ...
        const classRow = await db_1.pool.query('SELECT day_start_time FROM classes WHERE id = (SELECT class_id FROM sections WHERE id = $1) LIMIT 1', [section_id]);
       ...
       WHERE sc.school_id = $1 AND sc.start_date <= $2 AND sc.end_date >= $2 LIMIT 1`, [school_id, today]);
        ...
        for (const rec of records) {
            ...
            if (!student_id || !['present', 'absent'].includes(status))
                continue;
            await db_1.pool.query(`INSERT INTO attendance_records
         ...
         ON CONFLICT (section_id, student_id, attend_date) DO UPDATE
           SET status = EXCLUDED.status, submitted_at = now()`, [school_id, section_id, student_id, user_id, today, status]);
        }
...
router.patch('/today/:student_id', async (req, res) => {
    try {
        const { user_id, school_id } = req.user;
        const today = await (0, today_1.getToday)(school_id);
        ...
        const studentRow = await db_1.pool.query('SELECT id FROM students WHERE id = $1 AND section_id = $2 AND school_id = $3', [student_id, section_id, school_id]);
        if (studentRow.rows.length === 0)
            return res.status(403).json({ error: 'Student not in your section' });
        // Check if original record exists
        const existing = await db_1.pool.query('SELECT id, status, first_submitted_at FROM attendance_records WHERE student_id = $1 AND attend_date = $2 AND section_id = $3', [student_id, today, section_id]);
        const isLateArrival = status === 'present' && existing.rows[0]?.status === 'absent';
        const arrivedAt = isLateArrival ? new Date() : null;
        if (existing.rows.length > 0) {
            await db_1.pool.query(`UPDATE attendance_records
         SET status = $1,
             submitted_at = now(),
             edited_by = $2,
             edited_at = now(),
         ...
         WHERE student_id = $5 AND attend_date = $6 AND section_id = $7`, [status, user_id, isLateArrival, arrivedAt, student_id, today, section_id]);
        }
        else {
            await db_1.pool.query(`INSERT INTO attendance_records
           (school_id, section_id, student_id, teacher_id, attend_date, status,
            submitted_at, first_submitted_at, is_late, arrived_at)
         VALUES ($1, $2, $3, $4, $5, $6, now(), now(), $7, $8)`, [school_id, section_id, student_id, user_id, today, status, isLateArrival, arrivedAt]);
        }
        return res.json({
...
router.get('/:date', async (req, res) => {
    try {
        ...
        if ('error' in resolved)
            return res.status(resolved.status).json({ error: resolved.error });
        const { section_id } = resolved;
        const students = await db_1.pool.query(`SELECT s.id, s.name,
              ar.status AS attendance_status,
              ar.is_late,
              ar.arrived_at,
              ar.submitted_at
