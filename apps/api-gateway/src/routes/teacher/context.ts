...
router.get('/performance', async (req: Request, res: Response) => {
  try {
    ...
    const reasons = isSupporting ? [
      { factor: 'Attendance submission (35pts)', your_value: `${attScore}/35 — submitted ${me.attendance_days_month} out of ${schoolDays} working days this month`, school_avg: '—', impact: 'high' as const, status: (attScore >= 30 ? 'good' : attScore >= 20 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Homework sent to parents (25pts)', your_value: `${hwScore}/25 — sent homework ${me.homework_days_month} out of ${schoolDays} working days this month`, school_avg: '—', impact: 'high' as const, status: (hwScore >= 20 ? 'good' : hwScore >= 12 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Student observations (25pts)', your_value: `${obsScore}/25 — observed ${studentsObserved} out of ${totalStudents} students this month (need 1 obs per student)`, school_avg: '—', impact: 'medium' as const, status: (studentsObserved >= totalStudents ? 'good' : studentsObserved >= Math.ceil(totalStudents * 0.7) ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Class feed posts (15pts)', your_value: `${feedScore}/15 — posted ${feedDays} out of ${schoolDays} working days this month (1 post per working day needed)`, school_avg: '—', impact: 'medium' as const, status: (feed_pct >= 90 ? 'good' : feed_pct >= 60 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      ...
      { factor: 'Plan completion (40pts)',          your_value: `${planScore}/40 — completed plan ${me.completions_month} out of ${schoolDays} working days this month`, school_avg: '—', impact: 'high' as const,   status: (planScore >= 36 ? 'good' : planScore >= 28 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Attendance submission (20pts)',     your_value: `${attScore}/20 — submitted ${me.attendance_days_month} out of ${schoolDays} working days this month`, school_avg: '—', impact: 'high' as const, status: (attScore >= 18 ? 'good' : attScore >= 14 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Homework sent to parents (15pts)',  your_value: `${hwScore}/15 — sent homework ${me.homework_days_month} out of ${schoolDays} working days this month`, school_avg: '—', impact: 'medium' as const,  status: (hwScore >= 12 ? 'good' : hwScore >= 8 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Student observations (15pts)',      your_value: `${obsScore}/15 — observed ${studentsObserved} out of ${totalStudents} students this month (need 1 obs per student)`, school_avg: '—', impact: 'medium' as const, status: (studentsObserved >= totalStudents ? 'good' : studentsObserved >= Math.ceil(totalStudents * 0.7) ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
      { factor: 'Class feed posts (10pts)',           your_value: `${feedScore}/10 — posted ${feedDays} out of ${schoolDays} working days this month (1 post per working day needed)`, school_avg: '—', impact: 'low' as const,   status: (feed_pct >= 90 ? 'good' : feed_pct >= 60 ? 'warn' : 'bad') as 'good'|'warn'|'bad' },
    ];
