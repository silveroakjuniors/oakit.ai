...
router.get('/class-summary', async (req: Request, res: Response) => {
  try {
    ...
    if (sections.length === 0) return res.json({ total_students: 0, attendance: { present_count: 0, absent_count: 0, late_count: 0, school_days: 0, avg_attendance_pct: 0 }, attendance_trend: [], milestones_by_domain: [], observations_by_category: [], student_milestone_ranking: [], journal: { total_entries: 0, students_with_entries: 0 } });
    ...
    try {
      ...
      att = attendanceResult.rows[0] || {};
    } catch { /* ignore */ }

    // Attendance last 7 days (for line chart)
    let weeklyAttRows: any[] = [];
    try {
      ...
      weeklyAttRows = weeklyAttResult.rows;
    } catch { /* ignore */ }
    ...
    try {
      ...
      obsRows = obsResult.rows;
    } catch { /* ignore */ }

    // Students with most/least milestones achieved
    let studentRankRows: any[] = [];
    try {
      ...
      studentRankRows = studentMilestoneRank.rows;
    } catch { /* ignore */ }

    // Journal entries this month
    let journalData = { total_entries: 0, students_with_entries: 0 };
    try {
      ...
      journalData = journalResult.rows[0] || journalData;
    } catch { /* ignore */ }
    ...
    return res.json({
      ...
      attendance_trend: weeklyAttRows,
      milestones_by_domain: milestoneRows,
      observations_by_category: obsRows,
      student_milestone_ranking: studentRankRows,
      journal: journalData,
    });
  } catch (err) {
...
router.get('/student/:studentId', async (req: Request, res: Response) => {
  try {
    ...
    // Attendance this month
    let attData: any = { present: 0, absent: 0, late: 0, total_days: 0 };
    try {
      ...
      attData = attResult.rows[0] || attData;
    } catch { /* ignore */ }

    // Attendance last 30 days
    let attTrendRows: any[] = [];
    try {
    ...
    try {
      ...
      journalTotal = journalCount.rows[0]?.total || 0;
    } catch { /* ignore */ }

    const totalMilestones = milestoneRows.reduce((sum: number, r: any) => sum + r.total, 0);
    const achievedMilestones = milestoneRows.reduce((sum: number, r: any) => sum + r.achieved, 0);

    return res.json({
      student,
      attendance: {
        ...attData,
        pct: attData.total_days > 0 ? Math.round((attData.present / attData.total_days) * 100) : 0,
      },
      attendance_trend: attTrendRows,
      milestones_by_domain: milestoneRows,
      ...
      observations_by_category: obsRows,
      journal_entries_count: journalTotal,
    });
  } catch (err) {
