...
// GET /upcoming?page=1&limit=10 — upcoming holidays and special days for parent
router.get('/upcoming', async (req: Request, res: Response) => {
  try {
    const { school_id } = req.user!;
    const { getToday } = await import('../../lib/today');
    ...
    const holidays = await pool.query(
      `SELECT holiday_date::text AS date, event_name AS label, 'holiday' AS type
       FROM holidays WHERE school_id = $1 AND academic_year = $2 AND holiday_date >= $3
    ...
    const specials = await pool.query(
      `SELECT day_date::text AS date, label, day_type AS type, activity_note, duration_type
       FROM special_days WHERE school_id = $1 AND academic_year = $2 AND day_date >= $3
       ORDER BY day_date`,
      [school_id, academic_year, today]
    );

    const allEvents = [
      ...holidays.rows.map((r: any) => ({ date: r.date, label: r.label, type: 'holiday', note: null })),
      ...specials.rows.map((r: any) => ({ date: r.date, label: r.label, type: r.type, note: r.activity_note })),
    ].sort((a, b) => a.date.localeCompare(b.date));

    const total = allEvents.length;
    const paginated = allEvents.slice(offset, offset + limit);

    return res.json({ events: paginated, total, page, has_more: offset + limit < total });
  } catch (err) {
    console.error('[parent/calendar/upcoming]', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});
